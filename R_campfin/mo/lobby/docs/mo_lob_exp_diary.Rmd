...
### Read
After opening the files in a text editor, we can see that they are actually html tables despite the xls extension. 
```{r raw_read}

read_web_tb <- function(file){
  df <- read_html(file) %>% html_node("table") %>% html_table()
  return(df)
}

mo_lob_exp <- list.files(raw_dir, full.names = T)  %>% map_dfr(read_web_tb) %>% clean_names()
```

Most of the columns are read as text, and we will change the column types.

```{r col_type}
mo_lob_exp <- mo_lob_exp %>% 
  mutate(amount = str_remove(amount, "\\$") %>% as.numeric(),
         )
```
...
mo_lob_exp %>% 
  ...
  labs(
    title = "Missouri Lobbyist Expenditures by Year",
...
First, let's take a look at the distribution of the amount
```{r histogram}
 mo_lob_exp %>%
  ggplot(aes(amount)) +
  ...
  labs(
    ...
    subtitle = "attributable expenses made by lobbyists from 2004 to 2019",
    caption = "Source: Missouri Ethics Commission",
    x = "Amount",
    y = "Count"
  )
