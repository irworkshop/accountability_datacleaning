---
title: "Missouri Lobbying Registraruib Diary"
author: "Yanqi Xu"
date: "`r Sys.time()`"
output:
  github_document: 
...
```{r raw_dir}
raw_dir <- dir_create(here("mo", "lobby", "data", "raw"))
```
...
```{r na_flag}
mo_lob_reg <- mo_lob_reg %>% flag_na(company_name)
...
mo_lob_reg %>% 
  ...
  labs(
    title = "Missouri Lobbyists by Year",
...
progress <- progress_table(
  mo_lob_reg$CITY,
  mo_lob_reg$FIRM_CITY,
  mo_lob_reg$CITY_norm,
  mo_lob_reg$FIRM_CITY_norm,
  mo_lob_reg$CITY_swap,
mo_lob_reg$FIRM_CITY_swap,
  compare = many_city
...
```{r write_clean}
write_csv(
  x = mo_lob_reg %>% rename(CITY_clean = CITY_swap) %>% rename( FIRM_CITY_clean = FIRM_CITY_swap),
