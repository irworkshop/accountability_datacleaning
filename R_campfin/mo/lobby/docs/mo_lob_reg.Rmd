...
mo_lob_new <- mo_lob_new %>% 
  ...
  flag_dupes(-c(registration_year, date_registration), .both = F)
```

### Join
We can see that each `lob_id` responds to one lobbyist, so it's a unique id, which we can use to join the `mo_lob` and `mo_prin` dataframes.
...
mo_lob_reg_new <- mo_lob_new %>% mutate(date_registration = as.character(date_registration)) %>% 
  right_join(mo_prin_new %>% mutate(p_relationship_from = as.character(p_relationship_from)), 
            by = c("mecid_l" = "mecid_l", 
                  "date_registration" = "p_relationship_from"))
```
...
mo_lob_reg_new <- mo_lob_reg_new %>% 
  mutate(term_date = na_if(date_determined, "NULL") %>% as.numeric() %>% excel_numeric_to_date(date_system = "modern"))
```
...
write_csv(
  x = mo_lob_reg,
  path = path(clean_dir, "mo_lob_reg_clean.csv"),
  na = ""
)
```
