...
Microsoft Access Database (`.mdb`) files can not be easily read into R as data
frames. Instead, we will use a free and open source [`mdbtools`][mdbtools] 
command line tool to parse the tables as text files.

The `mdbtools` package can be installed on Debian based systems like so:
...
```{r na_flag}
aze <- aze %>% flag_na(date, last, amount, committee)
sum(aze$na_flag)
```
...
raw_in <- percent(prop_in(aze$city_raw, valid_city))
progress %>% 
...
dict_raw <- tibble(
  var = md_code(names(aze)),
  type = md_code(map_chr(aze, typeof)),
  def = c(
...
```{r dict_write}
write_lines(
  x = c("# Arizona Contracts Data Dictionary\n", dict_md),
  path = here("az", "expends", "az_expends_dict.md"),
)
```
