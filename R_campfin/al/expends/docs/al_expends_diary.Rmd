...
Then, we can use a list of [abbreviations from the USPS][05] to expand all abbreviations

[05]: https://pe.usps.com/text/pub28/28apc_002.htm
...
From the `zipcodes` database, we can create a comprehensive list of valid state abbreviations.

```{r valid_states, collapse=TRUE}
...
The next step involves matching a payee `city_prep` value to the _expect_ city name for that
payee's ZIP code and state.
...
al %>% 
  mutate(
    city_refine = city_swap %>% 
      key_collision_merge(dict = alabama_towns) %>% 
      n_gram_merge(numgram = 1),
    refined = (city_refine != city_swap)
  )
```
...
al %>% 
  filter(refined) %>% 
  filter(city_refine %out% valid_cities) %>% 
  count(city_refine) %>% 
  arrange(desc(n))
```
