...
[Mississippi Secretary of State](https://sos.ms.gov/elec/portal/msel2/page/search/portal.aspx) makes lobbyist and client registry searchable and downloadable from 2010 to 2020.
...
## Join
First， we noticed that the lobbyist fields in the client directory is jumbled together. We'll separate them first using `unnest_longer()`.
...
We can see that there are over 1,000 columns from each dataframe that doesn't have corresponding information from another one.
