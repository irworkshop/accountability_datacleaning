...
the `dateofreceipt` ranges from `r print(min_date)` to `r print(max_date)`.
1. There are `r sum(dc$na_flag)` records missing a `contributorname` or `amount` value (flagged 
...
1. The `yearofreceipt` variable contains the 4 digit year of the receipt.
1. There are not both names to _every_ transaction (`r not_na` of records have all data)
