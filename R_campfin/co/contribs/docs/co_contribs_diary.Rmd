...
```{r fig.keep=FALSE}
explore_plot(filter(coc, !is.na(receipt_type)), receipt_type)
explore_plot(filter(coc, !is.na(committee_type)), committee_type)
```
...
raw_in <- percent(prop_in(coc$city_raw, valid_city))
progress %>% 
...
1. There are `r comma(sum(coc$na_flag))` records missing ....
