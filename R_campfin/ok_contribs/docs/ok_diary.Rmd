...
```{r download_zip}
dir_create(here("ok_contribs", "data", "raw"))
if (any_old_files(here("ok_contribs", "data", "raw"), "*.zip")) {
  download.file(
    url = "https://www.ok.gov/ethics/public/dfile.php?action=csv",
    destfile = here("ok_contribs", "data", "raw", )
  )
}
```
...
```{r unzip}
if (any_old_files(here("ok_contribs", "data", "raw"), "*.csv")) {
  unzip(
    zipfile = here("ok_contribs", "data", "raw", "ethicscsvfile.zip"),
...
```{r read_contributor}
contributors <- 
  here("ok_contribs", "data", "raw", "contributor.csv") %>% 
...
```{r read_cont_type}
cont_types <-
  here("ok_contribs", "data", "raw", "cont_type.csv") %>% 
  read_csv(col_types = cols(.default = "c")) %>% 
  remove_empty("cols") %>% 
  clean_names()
```
...
individual_conts <-
  here("ok_contribs", "data", "raw", "individual_cont.csv") %>% 
  read_csv(col_types = cols(.default = "c")) %>% 
  remove_empty("cols") %>% 
  clean_names() %>%
  rename(
...
business_conts <- 
  here("ok_contribs", "data", "raw", "business_cont.csv") %>% 
  read_csv(col_types = cols(.default = "c")) %>% 
  remove_empty("cols") %>% 
  clean_names() %>% 
  rename(
...
committee_conts <-  
  here("ok_contribs", "data", "raw", "committee_cont.csv") %>% 
  read_csv(
...
```{r read_vendor_cont}
vendor_conts <-
  here("ok_contribs", "data", "raw", "vendor_cont.csv") %>% 
  read_csv(col_types = cols(.default = "c")) %>% 
  remove_empty("cols") %>% 
  clean_names() %>% 
...
```{r read_so1}
so1 <-
  here("ok_contribs", "data", "raw", "so1.csv") %>% 
  read_csv(
    col_types = cols(
...
```{r read_so2}
so2 <- 
  here("ok_contribs", "data", "raw", "so2.csv") %>% 
  read_csv(
    col_types = cols(
      .default = col_character(),
      ORGANIZATION_DATE = col_date("%m/%d/%Y")
...
```{r read_affiliation}
affiliations <- 
  here("ok_contribs", "data", "raw", "affiliation.csv") %>% 
  read_csv(col_types = cols(.default = "c")) %>% 
  remove_empty("cols") %>% 
  clean_names()
```
...
ballot_measures <-
  here("ok_contribs", "data", "raw", "ballot_measure.csv") %>% 
  read_csv(col_types = cols(REP_NUM = "c", BALLOT_YEAR = "d")) %>% 
  remove_empty("cols") %>% 
  clean_names()
```
...
```{r read_depository}
depositories <-
  here("ok_contribs", "data", "raw", "depository.csv") %>% 
  read_csv(col_types = cols(.default = "c")) %>% 
  remove_empty("cols") %>% 
  clean_names() %>% 
  rename(
    depos_dname  = full_name,
    depos_street = street,
    depos_city   = city,
    depos_state  = state,
    depos_zip    = zip
...
```{r read_party}
parties <- 
  here("ok_contribs", "data", "raw", "party.csv") %>% 
  read_csv(
    col_types = cols(
...
```{r read_office}
offices <- 
  here("ok_contribs", "data", "raw", "office.csv") %>% 
  read_csv(col_types = cols(.default = "c")) %>% 
  remove_empty("cols") %>% 
  clean_names() %>% 
  rename(office_id = id)
...
```{r read_district}
districts <-
  here("ok_contribs", "data", "raw", "district.csv") %>% 
  read_csv(col_types = cols(.default = "c")) %>% 
  remove_empty("cols") %>% 
  clean_names()
```
...
```{r read_candidate}
candidates <- 
  here("ok_contribs", "data", "raw", "candidate.csv") %>% 
  read_csv(col_types = cols(.default = "c")) %>% 
  remove_empty("cols") %>% 
  clean_names() %>% 
  rename(ethics_id = ethics_num)
...
```{r read_lump_fund}
lump_funds <- 
  here("ok_contribs", "data", "raw", "lump_fund.csv") %>% 
  read_csv(
    col_types = cols(
      .default = col_character(),
...
```{r read_report}
reports <- 
  here("ok_contribs", "data", "raw", "report.csv") %>% 
  read_csv(
    col_types = cols(
      .default = col_character(),
...
```{r read_rep_type}
rep_types <- 
  here("ok_contribs", "data", "raw", "report_type.csv") %>% 
  read_csv(col_types = cols(.default = "c")) %>%
  remove_empty("cols") %>% 
  clean_names()
```
...
all_contributors <- contributors %>%
  filter(cont_id %out% vendor_conts$cont_id)  %>% 
...
```{r}
ok <- ok %>% 
  left_join(all_contributors, by = "cont_id") %>% 
...
`trans_amount`. They will be flagged with a new `na_flag` variable.
...
First, we will prepare the `cont_city` string by removing punctuation and numbers, expanding
directional and geographical abbreviations ("N" for "North", "MT" for "MOUNT", etc), as well as
trimming and squishing excess white space. We will also remove common state abbreviations from the
city names.
...
are even more existing values that should really be interpreted as missing.
...
sum(is.na(ok$cont_city_prep)) - sum(is.na(ok$cont_city)) #10,000
```
...
too uncommon or unofficial to be included in our valid city list. "MIDWEST CITY" is really a part
of Oklahoma City but will be left unchanged. We only want to correct misspellings and remove
nonsense.
...
```{r}
ok_city_fix <- ok %>%
  # 0.953 in OK, don't confuse cities
  filter(cont_state_clean== "OK") %>% 
...
bad_fix %>% 
  left_join(zipcode, by = c("zip", "state")) %>%
  arrange(city) %>% 
  print_all()
```
