...
If a column is missing any values, it seems to be missing _most_ values.

```{r na_count}
col_stats(utc, count_na)
```

Since we included "Not Applicable" as a true `NA` value when reading the file,
around `r percent(prop_na(utc$vendor_name))` of `vendor_name` values are
missing.
...
We can greatly compress this external text file indicating duplicate rows.

```{r dupe_tar, eval=FALSE}
...
explore_plot(utc, deptartment) + scale_x_truncate()
