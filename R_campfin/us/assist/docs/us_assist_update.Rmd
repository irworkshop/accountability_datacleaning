...
fin_files <- "FY2020_All_Assistance_Full_20200913.zip"
...
```{r delta_path}
delta_file <- "FY(All)_All_Assistance_Delta_20200913.zip"
...
```{r}
s3 <- get_bucket_df(bucket = "publicaccountability")
s3 <- as_tibble(s3) %>% 
  filter(str_starts(Key, "csv/")) %>% 
  transmute(
    path = as_fs_path(Key),
