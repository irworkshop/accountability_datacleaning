...
```{r raw_dir}
raw_dir <- dir_create(here("ri", "lobby", "data", "raw"))
```
...
ri_lob_reg <- ri_lob_reg %>% mutate(DATE = excel_numeric_to_date(ri_lob_reg$DATE, date_system = "modern"))
```
...
head(ri_lob_reg)
tail(ri_lob_reg)
glimpse(sample_n(ri_lob_reg, 20))
```

### Missing

```{r na_count}
col_stats(ri_lob_reg, count_na)
```

```{r na_flag}
ri_lob_reg <- ri_lob_reg %>% flag_na(EMPLOYERNAME, FIRMADDR)
sum(ri_lob_reg$na_flag)
```

### Duplicates

```{r dupe_flag}
ri_lob_reg <- flag_dupes(ri_lob_reg, dplyr::everything())
sum(ri_lob_reg$dupe_flag)
```

### Categorical

```{r n_diri_lob_reginct}
col_stats(ri_lob_reg, n_distinct)
```

Visualize number of lobbyists registered each year.

```{r create year}
ri_lob_reg <- ri_lob_reg %>% 
  mutate(YEAR = year(DATE))

ri_lob_reg$YEAR %>% tabyl()
```
...
The `campfin::normal_city()` function is a good ri_lob_regart, again converting case,
...
progress <- progress_table(
  ri_lob_reg$CITY,
  ri_lob_reg$FIRM_CITY,
  ri_lob_reg$CITY_norm,
  ri_lob_reg$FIRM_CITY_norm,
  ri_lob_reg$CITY_swap,
ri_lob_reg$FIRM_CITY_swap,
  compare = many_city
...
1. There are `r nrow(ri_lob_reg)` records in the database.
1. There are `r sum(ri_lob_reg$dupe_flag)` duplicate records in the database.
1. The range and distribution of `amount` and `date` seem reasonable.
1. There are `r sum(ri_lob_reg$na_flag)` records missing either recipient or date.
...
```{r write_clean}
write_csv(
  x = ri_lob_reg %>% rename(CITY_clean = CITY_swap) %>% rename( FIRM_CITY_clean = FIRM_CITY_swap),
  path = path(clean_dir, "ri_lob_reg_clean.csv"),
  na = ""
)
```
