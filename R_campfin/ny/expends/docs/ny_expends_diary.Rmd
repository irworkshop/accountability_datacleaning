...
`r ncol(ny)` columns. This data frame contains data for campaign contributions, expenditures, 
...
ny <- ny %>% 
  mutate(
    city_norm = normal_city(
      city = str_replace("\\bNY\\b", "NEW YORK"), 
      geo_abbs = usps_city,
      st_abbs = c("NY", "DC"),
      na = na_city,
      na_rep = TRUE
...
ny <- ny %>% 
  mutate(
    city_final = city_refine %>% 
      str_replace("^BKLYN$", "BROOKLYN") %>% 
      str_replace("^NYC$", "NEW YORK") %>% 
      str_replace("^NF$", "NIAGARA FALLS") %>% 
      str_replace("^LIC$", "LONG ISLAND CITY") %>% 
      str_replace("")
  )
```
