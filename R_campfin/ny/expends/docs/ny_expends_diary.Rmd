...
1-800-458-3453. Each page costs 25¢ plus postage and copy orders must be prepaid.
> 
...
```{r select_not_na}
ny <- select(ny, names(ny)[map_dbl(ny, prop_na) < 0.989])
```
...
```{r flag_na}
ny <- ny %>% flag_na(corp, filer_id, amount, date)
sum(ny$na_flag)
mean(ny$na_flag)
```
...
ny <- ny %>% 
  ...
  left_join(geo, by = c("state_norm" = "state", "zip_norm" = "zip")) %>% 
...
We can use `campfin::is_abbrev()` to check whether our `city_refine` value is an abbreviation
for the expected city for that `state_norm` and `zip_norm` combination.

```{r view_final_bad}
ny_abbs <- ny %>%
  filter(city_refine %out% valid_city) %>% 
  select(state_norm, zip_norm, city_refine) %>% 
  drop_na(city_refine) %>% 
  left_join(geo, by = c("state_norm" = "state", "zip_norm" = "zip")) %>%
  rowwise() %>% 
  mutate(match_abb = is_abbrev(city_refine, city)) %>% 
  filter(match_abb) %>% 
  rename(city_abb = city)
```

```{r count_abbs}
ny_abbs %>% count(city_refine, city_abb, sort = TRUE)
```

```{r join_abbs}
ny <- ny %>% 
  left_join(ny_abbs) %>% 
  mutate(city_final = coalesce(city_refine, city_abb))
```

```{r progress_table, echo=FALSE}
progress_table <- tibble(
  stage = as_factor(c("raw", "norm", "swap", "refine", "abbrevs")),
  prop_good = c(
...
1. There are `r nrow(ny)` records in the database.
1. There are `r sum(ny$dupe_flag)` duplicate records in the database.
1. The range and distribution of `amount` seems reasonable. 
1. The `date` variable has been cleaned by removing bad dates in the new `date_flag` variable
1. There are `r sum(ny$na_flag)` records missing a neccesary value, mostly `amount`.
...
```{r proc_dir}
proc_dir <- here("ny", "expends", "data", "processed")
dir_create(proc_dir)
```
...
ny %>% 
  ...
  write_csv(
    path = glue("{proc_dir}/ny_expends_clean.csv"),
    na = ""
  )
```
