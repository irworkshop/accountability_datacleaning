...
## Reshape
When we map the columns for The Accountability Project, we can reshape the data so that each record is an individual lobbyist. Right now it's arranged by lobbying firms, with all lobbyist in the same line.
```{r}
ny_lob_clean <- ny_lob %>% 
  mutate(individual_lobbyist_registered = str_split(additional_lobbyist_lr, pattern = ", ")) %>% 
  unnest_longer(individual_lobbyist_registered)
```
...
```{r write_clean}
write_csv(
  x = ny_lob_clean  %>% rename(lobbyist_city_clean = lobbyist_city_swap) %>% rename( client_city_clean = client_city_swap),
