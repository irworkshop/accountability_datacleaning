...
The [State of Wisconsin Ethics Commission][04] makes available directories of lobbyists, principals (otehrwise known as lobbying organizations, any person who employs a lobbyist) and state agency officials (legislative liaisons) that
lobbying for their agency. 
...
liasons <- file.path(raw_dir, "liasons")

dir_create(c(principals,lobbyists,liasons))

```

[04]:https://lobbying.wi.gov/Directories/DirectoryOfLicensedLobbyists/2019REG

```{r download all files, eval = FALSE}

wi_url_lobbyists <- glue("https://lobbying.wi.gov/Reports/Report.aspx?ReportPath=/GAB/Lobbying/Public&ReportName=DirectoryOfLicensedLobbyistsFlat&SessionID={(2013:2020)[c(TRUE,FALSE)]}REG&SessionStartYear={(2005:2020)[c(TRUE,FALSE)]}&SessionEndYear={(2013:2020)[c(FALSE,TRUE)]}&OutputFormat=Excel")
...
wi_url_principals <- glue("https://lobbying.wi.gov/Reports/Report.aspx?ReportPath=/GAB/Lobbying/Public&ReportName=DirectoryOfRegisteredLobbyingOrganizationsFlatter&SessionID={(2013:2020)[c(TRUE,FALSE)]}REG&SessionStartYear={(2013:2020)[c(TRUE,FALSE)]}&SessionEndYear={(2013:2020)[c(FALSE,TRUE)]}&OutputFormat=Excel")
  ...
  if (!all_files_new(lobbyists)) {
    download.file(
      ...
      destfile = glue("{lobbyists}/wi_lobbyists_{(2005:2020)[c(TRUE,FALSE)]}.xls"))
  }
  ...
  if (!all_files_new(principals)) {
    download.file(
      ...
      destfile = glue("{principals}/wi_principals_{(2005:2020)[c(TRUE,FALSE)]}.xls"))
  }


  if (!all_files_new(liasons)) {
    download.file(
      #this step execute strings as vairables.
      url = wi_url_liasons,
      destfile = glue("{liasons}/wi_liasons.xlsx"))
  }
...
## Reading
We'll first merge each dataset in each folder into a master dataset and then combine into one the three master datasets, `wi_principals` for principals, `wi_lobbyists` for lobbyists, and `wi_liasons` for legislative liasons. 
...
# libxls is not capable of reading the wi_liasons.xls file. Open the xls file in Excel, save it as xlsx file first.
wi_liasons <- read_xlsx(glue("{liasons}/wi_liasons.xlsx"),.name_repair = make_clean_names) %>% 
  mutate(end_date = excel_numeric_to_date(as.numeric(end_date),date_system = "modern"),
...
wi_liasons %>% glimpse_fun(count_na)
```
...
### Phone
```{r normal phone}
wi_lobbyists <- wi_lobbyists %>% mutate(phone_1_norm = normal_phone(phone_1),
                                        phone_2_norm = normal_phone(phone_2))
wi_principals <- wi_principals %>% mutate(phone_1_norm = normal_phone(principal_phone_1),
                                        phone_2_norm = normal_phone(principal_phone_2))
```
...
wi_principals <- wi_principals %>% mutate(principal_ZIP5 = normal_zip(principal_mailing_zip))
...
check_city <- function(city_input, verbose = FALSE) {
  ### Put your api_key here                                            
  response <- GET(api_url_root, query = list(query = city_input, key = api_key))
  ...
  warning("You must use an API key", call. = FALSE)
      ...
      formatted_adress = r_content$results[[1]]$formatted_address
      city_fetched = str_split()
    }
}
