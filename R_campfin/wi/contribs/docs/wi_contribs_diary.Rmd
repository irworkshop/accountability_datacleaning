...
Data is obtained from the Wisconsin Secretary of State's Campaign Finance System
(CIFS).
...
Using the CIFS [contribution search portal][search], we can run a search for
...
raw_file <- path(raw_dir, "ReceiptsList.csv")
```
...
wic <- vroom(
  ...
  col_types = cols(
    ...
    # TransactionDate = col_date_usa(),
    # ContributionAmount = col_double(),
    # `72 Hr. Reports` = col_skip(),
...
```{r na_flag}
wic <- wic %>% flag_na(date, last, amount, committee)
sum(wic$na_flag)
```
...
1. There are `r comma(sum(wic$na_flag))` records missing ....
...
write_csv(
  x = wic,
  path = path(clean_dir, "wi_contribs_clean.csv"),
  na = ""
)
```
