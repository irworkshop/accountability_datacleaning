...
```{r plot gaps, warning=FALSE, message=FALSE}
...
```{r plot expends, warning=FALSE, message=FALSE}
wi_lobby_expends %>% 
...
```{r write guess_city table, echo=FALSE}
wi_lobbyist_out %>% 
  write_csv(
    path = glue("{clean_reg_dir}/wi_lobbyist_out.csv"),
    na = ""
  )
wi_principal_out %>% 
  write_csv(
    path = glue("{clean_reg_dir}/wi_principal_out.csv"),
    na = ""
  )
```
