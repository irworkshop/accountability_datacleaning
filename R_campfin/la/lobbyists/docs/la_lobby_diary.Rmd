...
la_lines <- la_lines %>% na_if("^\\s/\\s$") %>% na.omit()
for (i in seq_along(la_lines)) {
  if (is.na(la_lines[i])) {
    next()
  }
  if (str_detect(la_lines[i],"^ / d/b/a ")) {
    la_lines[i-1] <- str_c(la_lines[i-1], str_remove(la_lines[i], "^ / d/b/a "), collapse = ", ")
    la_lines[i] <- NA_character_
  }
}

la_lines[310] <- str_c(la_lines[310:315] %>% str_remove("^\\s/\\s"), collapse = " ")
la_lines[311:315] <- rep(NA_character_, 5)
#la_lines <- la_lines[-c(311:315)]

#la_lines <- la_lines %>% na.omit()
regex <- "^\\s/\\s"
# for (i in seq_along(la_lines)) {
# if (str_detect(la_lines[i],regex)) {
#   la_lines[i-1] <- str_c(la_lines[i-1], str_remove(la_lines[i], sep), sep = ", ")
#   #test <- test[-i]
#   la_lines[i] <- NA_character_
#   #test <- test %>% na.omit()
# }  
...
for (i in seq_along(la_lines)) {
  ...
  if (str_detect(la_lines[i],regex)) {
    la_lines[i-1] <- str_c(la_lines[i-1], str_remove(la_lines[i], regex), collapse = "")
    la_lines[i] <- NA_character_
  }
}
...
# for (i in seq_along(la_lines)) {
#   if (str_detect(la_lines[i],"^\\s/\\s")) {
#     la_lines[i-1] <- str_c(la_lines[i-1], str_remove(la_lines[i], "^\\s/\\s"), collapse = " ")
#     la_lines[i] <- NA_character_
#   }
# }
...
la_reg <- la_lines %>% na.omit() %>%  read_csv()

prob <- problems(la_reg)

la_reg <- tibble::rowid_to_column(la_reg)
