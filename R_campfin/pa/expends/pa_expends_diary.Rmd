...
There are `r sum(pa$na_flag)` columns in city_state_zip that are NAs
```{r na_flag}
...
pa %>%   drop_na(EXPDESC) %>% 
  ...
  geom_col() +
  labs(title = "Pennsylvania Campaign Expenditures by Average Spending",
...
### Dates
The dates seem to be reasonable, with records dating back to `r summary(pa$date)[1]` till `r summary(pa$date)[-1]` 
```{r}
summary(pa$EXPDATE)
```
...
pa_match_table$city_match <- pa_match_table$city_match %>% 
  ...
  str_replace("CRANBERRY", "CRANBERRY TOWNSHIP")
...
pa <-pa %>% 
  left_join(select(pa_match_match, index, sec_city_match), by = "index") %>% 
...
```{r}
pa_sec_refined <- pa %>%
  filter(city_clean %out% c(valid_city, pa_match_table$sec_city_match)) %>% 
  mutate(
    city_sec_refine = city_clean %>% 
      key_collision_merge(dict = c(valid_city, pa_match_table$sec_city_match), keep_original) %>% 
      n_gram_merge(numgram = 1),
    refined = (city_clean != city_sec_refine)
  ) %>% 
  filter(refined) %>% 
  select(
    index,
...
```{r}
cities <- select(pa_match_table, STATE, sec_city_match) %>% rename("state" = "STATE") %>% rename("city" = "sec_city_match") %>% rbind(select(zipcode, -zip)) %>% unique()
# Get a table of cities that are not in the valid_city vector
pa_out <- pa %>% 
  count(STATE, city_clean, sort = TRUE) %>% 
  filter(city_clean %out% cities$city) %>% 
  drop_na()

# Fuzzy-matching city names with the names out of such list. This will take a while, and be patient
prep_refine <- pa_out %>% 
  # Join on all names in the relevant state
  left_join(cities, by=c(STATE = "state")) %>%
  # Calculate the distances, per original city name.
  group_by(city) %>%                
  mutate(dist = diag(adist(city_clean, city, partial=TRUE))) %>%
  # Append the agrepl result with the Levenshtein edit distance
  rowwise() %>%
  mutate(string_agrep = agrepl(city_clean, city, max.distance = 0.3)) %>%
  ungroup() %>%  
