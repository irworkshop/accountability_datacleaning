...
```{r}
valid_city <- unique(c(valid_city, pa_match_table$sec_city_match), "WYOMISSING")
...
```
We also need to process the filer table to join back to the `FILERID` field.
```{r unzip filer, eval=FALSE}
if (all_files_new(path = raw_dir, glob = "*.txt")) {
  for (i in seq_along(zip_files)) {
    unzip(
      zipfile = zip_files[i],
...
valid_places <- unique(c(valid_city, extra_city, pa_filer_out$city[pa_filer_out$check_city_flag]))
```

```{r fetch_city, eval=FALSE}
pa_filer <- pa_filer %>% mutate(city_fetch = city %>% 
  str_replace("^PHILA$", "PHILADELPHIA") %>% 
  str_replace("^PGH$", "PITSSBURGH"))

pa_filer_fetch <- pa_filer %>% filter(city_fetch %out% valid_places)
```

```{r}
#write pa_filer_clean to a csv. 
