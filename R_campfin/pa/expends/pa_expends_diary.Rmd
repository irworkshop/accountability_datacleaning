...
subtitle: "Pennsylvania Expenditures August 1"
author: "Yanqi Xu"
date: "`r Sys.time()`"
output:
  github_document: 
...
Download raw, **immutable** data file. Go to https://www.dos.pa.gov//VotingElections/CandidatesCommittees/CampaignFinance/Resources/Documents/2019.zip,
...
```{r download to raw_dir, echo=FALSE}
...
pa <- expense_files %>% map(read_csv, col_names = pa_col_names) %>% bind_rows()

s
read_c
n_delim <- str_count(pa_lines[1], ",")
  map(
    read_delim,
    delim = ",",
    escape_double = FALSE,
    escape_backslash = FALSE,
    col_names = c("FILERID","EYEAR","CYCLE","EXPNAME","ADDRESS1","ADDRESS2","CITY","STATE","ZIPCODE","EXPDATE","EXPAMT","EXPDESC"),
    col_types = cols(
...
pa %>% 
  ...
  geom_col() +
    labs(title = "Pennsylvania Campaign Expenditures",
...
```{r na_flag}
pa <- pa %>% mutate(na_flag = is.na(city_state_zip))
```

### Duplicates
There are no duplicates
```{r get_dupes, collapse=TRUE}
pa_dupes <- get_dupes(wy)
```
...
pa %>% 
  ...
  labs(
    title = "Pennsylvania Expenditure Amount Ranges",
    x     = "Expenditure Type",
    y     = "Amount",
    caption = "Source: Pennsylvania Dept. of State"
  )
...
```{r add_year}
pa <- pa %>% mutate(year = year(date), on_year = is_even(year))
```
...
### Indexing
```{r}
pa <- tibble::rowid_to_column(wy, "id")
```
...
pa <- pa %>% mutate(state_clean = normal_state(state_clean))
```
...
pa <- pa %>% mutate(city_clean=ifelse(is.na(city_clean)==TRUE, 
               str_extract(city_state_zip, "^[A-Z]{2,}\\S$"),paste(city_clean)))

count_na(wy$city_clean)
## wy_ZIP <- pa %>% str_split(wy$city_state_zip, "\\s\\d{5}")
```

`r sum(!is.na(wy$city_clean))` cities were found.
```{r prep_city}
pa <- pa %>% mutate(city_prep = normal_city(city_clean))
...
```{r}
pa_refined$city_refine <- wy_refined$city_refine %>% 
...
There are `r sum(!is.na(wy$city_clean))` entries of cities identified in the original data matching the regex with `r n_distinct(wy$city_clean)` distinct values, after the swap and refine processes, there are `r sum(!is.na(wy$city))` entries with `r n_distinct(wy$city)` distinct values. 
