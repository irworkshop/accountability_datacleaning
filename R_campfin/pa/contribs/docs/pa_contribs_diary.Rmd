...
```{r raw_fix}
fix_check <- here("pa", "contribs", "fixed.txt")
...
using `purrr::map_df()` and `readr::read_csv()`. We need to use the column
names and types listed in the `readme.txt` file we downloaded earlier.

```{r raw_read}
pac <- map_df(
  .x = sample(con_paths, 2),
  .f = read_delim,
  delim = ",",
  escape_backslash = FALSE, 
  escape_double = FALSE,
  col_names = str_to_lower(readme$contribs$col),
  col_types = cols(
    .default = col_skip(),
...
```{r na_flag}
pac <- pac %>% flag_na(date, last, amount, committee)
sum(pac$na_flag)
```
