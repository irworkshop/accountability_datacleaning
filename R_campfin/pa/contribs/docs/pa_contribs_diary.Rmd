...
If this has already been done, it's easier to read the single file that was
...
If there are no consolidated contribution files from cleaning, we will have to
read and join the files from scratch.
...
needed. This is the step that is done above for the `no_pac` check.
...
data frame into a list of 100,000 row chunks and check each chunk at a tinme,
...
We can now read that file as a vector and add it as a new `dupe_flag` variable.
...
To upload this duplicates text file to GitHub, we will convert it to an string
of binary integers and compress it.

```{r dupe_compress, eval=FALSE}
...
```{r violin_amount_party, echo=FALSE}
pac %>%
...
dict_raw <- tibble(
  ...
  def = c(
    ...
    "Normalized contributor state abbreviation",
