...
emp_urls <- paste0('https://www.in.gov',urls[str_detect(urls, 'Employer Lobbyist Total')] %>% html_attr('href'))
...
```{r read csv}
emp_dir <- path(raw_dir,"emp")
inle_csv <- read_csv(dir_ls(emp_dir) %>% str_subset('csv'), 
                 col_types = cols(.default = col_character())) %>% 
  clean_names() 

in_0809_fs <- dir_ls(emp_dir) %>% str_subset('2008|2009')

in_1112_fs <- dir_ls(emp_dir) %>% str_subset('2011|2012')

in_1319_fs <- dir_ls(emp_dir) %>% str_subset('2013|2014|2015|2016|2017|2019') %>% .[1:6]
...
in_1319 <- map_dfr(in_1319_fs[1:6], read_inxl)
  # bind_rows(inle_csv)
...
Next, we can see that the file strutures for 2008 and 2009 are different from the one later on. The main difference is that: 
