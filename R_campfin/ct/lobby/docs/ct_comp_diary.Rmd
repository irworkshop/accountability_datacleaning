...
This Rmd file documents the CT compensation data only, whereas the registration data is wrangled in a separate data diary.
...
ctlc %>% filter(client %in% top5$client) %>% 
  ...
  labs(
    title = "Conencticut Top 10 Clients Lobbhying Amount Distribution",
...
```{r sankey plot, echo=FALSE}
links <- ctlc %>% filter(client %in% top5$client) %>% select(client, communicator,total) %>% 
  reorde
nodes <- data.frame("client"=links %>% pull(client) %>% unique())

sankeyNetwork(Links = links, Nodes = nodes,
 Source = "client", Target = "communicator",
 Value = "total", NodeID = "client",
...
```
Our goal is to reduce the number of rows in the `ct_reg` table to `r nrow(ct_count)`, which is the total number of ct_reg tows (each row represents a distinct relationship between a client and a lobbyist for a session).
...
ct_join<- ct_join %>% filter(n !== 2) %>% 
  select(-c(row_sum, early_more_info)) %>% 
  unite(remove = T, col = lobbyist_first_name, lobbyist_last_name,sep = ",") %>% 
...
ctlc <- ctlc %>% 
  ...
  select(-c(dupe_flag, source_file)
...
```{r write clean}
clean_dir <- here("ct", "lobby", "data", "raw","exp")
