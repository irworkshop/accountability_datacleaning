...
We can check to ensure the file was properly read by counting the number of
values from a discrete variable like `amended`.
...
arc %>%
  ...
  labs(
    title = "Arkansas Amount Distribution",
...
We can use the `year()` function to create a 4-digit year from the `date`. 
