...
```{r raw_read}
illr <- as_tibble(read.csv(file = path(raw_dir, "LR_LOBBYIST.csv"), stringsAsFactors = FALSE, fileEncoding = 'UTF-16LE'))
illr_ent <- as_tibble(read.csv(file = path(raw_dir, "LR_LOBBYING_ENT.csv"), stringsAsFactors = FALSE, fileEncoding = 'UTF-16LE'))
...
```{r raw_join}
mean(illr$agent_name %in% agent$agent_name)
illr <- left_join(illr, agent)
```
...
```{r na_flag}
illr <- illr %>% flag_na(active_date, agent_name, firm_name, client_name)
sum(illr$na_flag)
```
...
```{r date_year}
illr <- mutate(illr, year = year(active_date))
```

```{r date_range}
min(illr$active_date)
max(illr$active_date)
sum(illr$active_date > today())
```
...
1. There are `r comma(sum(illr$dupe_flag))` duplicate records in the database.
...
dict_raw <- tibble(
  var = md_code(names(illr)),
  type = md_code(map_chr(illr, typeof)),
  def = c(
    "Lobbyist full name",
    "Unique firm ID",
    "Firm name",
