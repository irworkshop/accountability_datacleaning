...
`r percent(prop_in(ma$city_final, valid_city, na.rm = TRUE))`, above our 99% goal. There are still
`r length(setdiff(ma$city_final, valid_city))` different _invalid_ values that could be checked, but
...
progress_table <- tibble(
  ...
  prop_good = c(
    prop_in(ma$city_raw, valid_city, na.rm = TRUE),
    prop_in(ma$city_norm, valid_city, na.rm = TRUE),
    prop_in(ma$city_swap, valid_city, na.rm = TRUE),
    prop_in(ma$city_refine, valid_city, na.rm = TRUE),
    prop_in(ma$city_final, valid_city, na.rm = TRUE)
  ),
  unique_bad = c(
    length(setdiff(ma$city_raw, valid_city)),
    length(setdiff(ma$city_norm, valid_city)),
    length(setdiff(ma$city_swap, valid_city)),
    length(setdiff(ma$city_refine, valid_city)),
    length(setdiff(ma$city_final, valid_city))
  )
)
