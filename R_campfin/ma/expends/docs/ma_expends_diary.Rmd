...
The `zip` address is already fairly clean, with `r percent(prop_in(ma$zip, geo$zip, na.rm = TRUE))`
of the values already in our comprehensive `geo$zip` list.
...
This brings our valid percentage to `r percent(prop_in(ma$zip_norm, geo$zip, na.rm = TRUE))`.
...
`r percent(prop_in(ma$state, geo$state, na.rm = TRUE))`.
...
There are still `r length(setdiff(ma$state, geo$state))` invalid values which we can remove.
...
`r percent(prop_in(ma$city, geo$city, na.rm = TRUE))` already in `geo$city` (which is not
...
This process brought us to `r percent(prop_in(ma$city_norm, geo$city, na.rm = TRUE))` valid.
...
`r percent(prop_in(ma$city_swap, geo$city, na.rm = TRUE))` and reduce the number of distinct
_invalid_ values from `r length(setdiff(ma$city_norm, geo$city))` to only
`r length(setdiff(ma$city_swap, geo$city))`
...
This brings us to `r percent(prop_in(ma$city_refine, geo$city, na.rm = TRUE))` valid values.
...
`r percent(prop_in(ma$city_final, geo$city, na.rm = TRUE))`, above our 99% goal. There are still
`r length(setdiff(ma$city_final, geo$city))` different _invalid_ values that could be checked, but
...
progress_table <- tibble(
  ...
  total_distinct = c(
    n_distinct(ma$city_raw),
    n_distinct(ma$city_norm),
    n_distinct(ma$city_swap),
    n_distinct(ma$city_refine),
    n_distinct(ma$city_final)
  ),
  prop_good = c(
    prop_in(ma$city_raw, geo$city, na.rm = TRUE),
    prop_in(ma$city_norm, geo$city, na.rm = TRUE),
    prop_in(ma$city_swap, geo$city, na.rm = TRUE),
    prop_in(ma$city_refine, geo$city, na.rm = TRUE),
    prop_in(ma$city_final, geo$city, na.rm = TRUE)
  ),
  unique_bad = c(
    length(setdiff(ma$city_raw, geo$city)),
    length(setdiff(ma$city_norm, geo$city)),
    length(setdiff(ma$city_swap, geo$city)),
    length(setdiff(ma$city_refine, geo$city)),
    length(setdiff(ma$city_final, geo$city))
  )
)
