...
mdb_file <- unzip(zip_file, exdir = raw_dir)
```

```{r mdb_tables, results='asis'}
...
raw_in <- prop_in(mac$city_raw, vald_city)
progress %>% 
...
1. There are `r nrow(mac)` records in the database.
1. There are `r sum(mac$dupe_flag)` duplicate records in the database.
1. The range and distribution of `amount` and `date` seem reasonable.
1. There are `r sum(mac$na_flag)` records missing either recipient or date.
