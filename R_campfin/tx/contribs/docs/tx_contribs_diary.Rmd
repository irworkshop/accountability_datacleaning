...
Data is ontained from the [Campaign Finance section of the TEC website][cf]. An
entire database can be downloaded as [a ZIP file][zip]. The contents of that ZIP
and the layout of the files within are outlined in the [`CFS-ReadMe.txt`][rm]
file.
...
layout <- readme[130:168] %>%
  ...
  select(1, 2, 5) %>% 
  mutate(`Field Name` = make_clean_names(`Field Name`)) %>%
  mutate_at(vars(Type), str_remove_all, "[^A-z]")

layout$`Field Name` <- md_code(layout$`Field Name`)
kable(layout)
```
...
txc <- txc %>% 
  ...
  rename(
    ...
    filer = fil_name,
    date = dt,
    con_type = con_persent_type,
    con_org = con_organization,
    con_zip = con_postal_code,
    con_job = con_job_title,
    pac = con_oos_pac
  ) %>% 
  mutate_if(
    .predicate = ~all(unique(.) %in% c("Y", "N")),
    .funs = ~equals(., "Y")
  )
```
...
1. There are `r comma(sum(txc$dupe_flag))` duplicate records in the database.
1. The range and distribution of `amount` and `date` seem reasonable.
1. There are `r comma(sum(txc$na_flag))` records missing ....
