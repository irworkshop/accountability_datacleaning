...
The city value starts out very clean, with 
...
There are still many `city_final` values not in `geo$city`, but many of these are actually valid
cities simply not on our list. Others, like "SA" and "TX CITY" should be "SAN ANTONIO" and "TEXAS
CITY" (two valid cities).
...
progress_table <- tibble(
  ...
  prop_good = c(
    ...
    prop_in(tx$city_norm, geo$city, na.rm = TRUE),
    prop_in(tx$city_swap, geo$city, na.rm = TRUE),
    prop_in(tx$city_final, geo$city, na.rm = TRUE)
  ),
  unique_bad = c(
    length(setdiff(tx$payee_street_city, geo$city)),
    length(setdiff(tx$city_norm, geo$city)),
    length(setdiff(tx$city_swap, geo$city)),
    length(setdiff(tx$city_final, geo$city))
  )
)
...
`r percent(progress_table$prop_good[4])`. The number of total distinct city values descreased from
