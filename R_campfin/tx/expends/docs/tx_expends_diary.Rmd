...
```{r expanded_prop_in, collapse=TRUE}
...
progress_table <- tibble(
  ...
  unique_bad = c(
    length(setdiff(tx$payee_street_city, geo$city)),
    length(setdiff(tx$city_norm, geo$city)),
    length(setdiff(tx$city_swap, geo$city)),
    length(setdiff(tx$city_refine, geo$city)),
    length(setdiff(tx$city_final, expanded_valid))
  )
)
...
`r percent(progress_table$prop_good[5])`. The number of total distinct city values descreased from
