...
fs::dir_create(here::here("tx", "expends", "docs"))
```
...
readme[497:535] %>%
  ...
  str_c(collapse = "\n") %>%
  read_delim(delim = "|") %>%
  mutate(`Len Description` = coalesce(`Len Description`, Mask)) %>% 
  separate(
    col = `Len Description`,
    sep = " ",
  ...
  select(1, 2, 5) %>% 
  mutate(`Field Name` = md_code(to_snake_case(`Field Name`))) %>%
  mutate_at(vars(Type), str_remove_all, "[^A-z]") %>% 
  kable("markdown")
```
...
rm(zip_contents, readme, zip_expends)
```
...
tx <- vroom(
  file = dir_ls(raw_dir, glob = "*expend_\\d+.csv"),
...
```{r flag_dupes, collapse=TRUE, eval=FALSE}
...
```{r}
tx <- mutate(tx, dupe_flag = FALSE)
```

```{r collect_garbage, echo=FALSE}
gc()
```
...
The `expend_amount` value ranges from a `r dollar(min(tx$expend_amount, na.rm = TRUE))` minimum to
`r dollar(max(tx$expend_amount, na.rm = TRUE))`, with only 
`r sum(tx$expend_amount < 0, na.rm = TRUE)` records having a value less than \$0.
...
The logarithm of `expend_amount` is normally distributed around the median value of
`r dollar(median(tx$expend_amount, na.rm = TRUE))`.
...
tx %>%
  ...
  geom_vline(xintercept = median(tx$expend_amount, na.rm = TRUE)) +
  labs(
...
tx %>% 
  filter(!is.na(expend_yr), expend_yr > 1999) %>%
  count(expend_yr) %>% 
  mutate(even = is_even(expend_yr)) %>% 
  ggplot(aes(x = expend_yr, y = n)) +
...
tx %>% 
  mutate(
    expend_mn = month(expend_dt),
    even = is_even(expend_yr)
  ) %>% 
  group_by(even, expend_mn) %>% 
  summarise(median = median(expend_amount)) %>% 
  ggplot(aes(expend_mn, median)) +
...
```{r address_normal, collapse=TRUE, eval=TRUE}
tx <- tx %>% 
  # combine street addr
  unite(
...
The city value starts out fairly clean, with 
`r percent(prop_in(str_to_upper(tx$payee_street_city), geo$city, na.rm = TRUE))` of the
`payee_street_city` values already in our `geo$city` list (after only converting to uppercase).

#### Normalize

We can use `campfin::normal_city()` to perform some standard text normalization like converting
to uppercase, removing punctuation and numbers, and fixing white space erros. The function can also
remove state abbreviation from the end of city names (`"Austin, TX"`) and remove some common 
invalid values.
...
To get over 99% valid city names, we can expand our list of valid cities and a few confident
manual tweaks to `city_refine`. These tweaks will replace some common abbreviations.
...
1. There are `r sum(tx$dupe_flag)` duplicate records (`r percent(mean(tx$dupe_flag))`).
1. The range and distribution of `expend_amount` and `expend_dt` seem reasonable.
1. There are `r sum(tx$na_flag)` records missing important values (`r percent(mean(tx$na_flag))`).
