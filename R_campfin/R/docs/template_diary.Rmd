...
```{r get_dupes}
df_dupes <- distinct(get_dupes(df))
```
...
n_distinct(df$city)
prop_in(df$state, geo$city, na.rm = TRUE)
length(setdiff(df$city, geo$state))
```
...
length(setdiff(df$city_norm, geo$state))
```
