---
title: "Nursing Home Compare Data Diary"
author: "Yanqi Xu"
date: "`r Sys.time()`"
output:
  github_document: 
...
The abbreiviation information is stored in `NHC_DataDictionary_SNFQRP.pdf` and the record layouts are contained in `DataMedicareGov_MetadataAllTabs_v23.xlsx`. 
...
```{r read owner}
health <- read_csv(str_subset(path(raw_dir, zip_data),"HealthDeficiencies.+")) %>% clean_names()
```
...
First, we will read the ownership data. We can also view the record layout from the `Ownership` sheet `DataMedicareGov_MetadataAllTabs_v23.xlsx`.
...
```{r read owner dict}
health_dict <- read_xlsx(str_subset(path(docs, zip_others),"DataMedicareGov"),
                        sheet = "Ownership")

kable(health_dict)

tag_table <- health %>% select(tag, tag_desc) %>%  unqiue()
...
write_csv(
  x = tag_table,
  path = path(docs, "nursing_infection_clean.csv"),
  na = ""
)
```
