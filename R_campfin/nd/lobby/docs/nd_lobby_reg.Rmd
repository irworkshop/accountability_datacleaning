...
### State
The state fields is pretty clean and don't need to be normalized.
```{r clean state}
prop_in(nd_lobby_reg$state, valid_zip, na.rm = TRUE) %>% percent()
prop_in(nd_lobby_reg$org_state, valid_zip, na.rm = TRUE) %>% percent()
...
nd_lobby_reg <- nd_lobby_reg %>% mutate_at(
...
progress_table(nd_lobby_reg$city,
               nd_lobby_reg$city_norm,
               nd_lobby_reg$org_city,
               nd_lobby_reg$org_city_norm,
               compare = valid_place)
)
```
