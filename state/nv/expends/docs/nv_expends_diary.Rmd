...
doc_dir <- fs::dir_create(here::here("nv", "expends", "docs"))
```
...
`r comma(nrow(nv$Expenses))` expenditures made to committees and reported to the
state. This table does not identify the spending committee or receiving entity.
...
candidate or committee, each listed in their own table with their own key column
in Expenses. We can combine these keys and tables.
...
raw_in <- percent(prop_in(nve$city, valid_city))
progress %>% 
...
There are `r comma(nrow(nve))` rows of `r ncol(nve)` columns. Each record
represents a single expenditure between a committee and an outside vendor.
...
explore_plot(nve, expense_type) + scale_x_wrap()
explore_plot(nve, recipient_type) + scale_x_wrap()
```
...
nve %>%
  ...
  labs(
    title = "Nevada Expenditures Amount Distribution",
...
We can add the calendar year from `expense_date` with `lubridate::year()`
...
nve %>% 
  ...
  theme(legend.position = "bottom") +
  labs(
    title = "Nevada Expenditures by Year",
    caption = "Source: Nevada Secretary of State",
    fill = "Election Year",
    x = "Year Made",
    y = "Count"
...
1. There are `r comma(nrow(nve))` records in the database.
1. There are `r comma(sum(nve$dupe_flag))` duplicate records in the database.
1. The range and distribution of `amount` and `date` seem reasonable.
1. There are `r comma(sum(nve$na_flag))` records missing key variables.
