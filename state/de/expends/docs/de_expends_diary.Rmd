...
doc_dir <- fs::dir_create(here::here("de", "expends", "docs"))
```
...
This data is obtained from the Delaware [Campaign Finance Reporting System][03].

On the [Expenditures search page][04], the CFRS describes what can be searched:
...
On [the FAQ page][05], the CFRS defines exactly what expenditures are reported.
...
We can download the raw file using a combination of `POST()` and `GET()`.
...
First, we use `httr::POST()` to fill out the CFRS form to return all results
from January 1, 2000 to today.
...
Before reading the file, we need to use regular expressions to fix two big
problems in the raw text file:
1. EQuation marks being used where they should be apostrophes
2. Errant newline characters in the middle of lines
...
If the file was properly fixed and read, we can count the distinct values of a
known discrete variable like the logical `Fixed Asset`.

```{r raw_check}
count(dee, `Fixed Asset`)
```

For convenience, we can rename variables and remove the last empty column.
...
We can use `campfin::flag_na()` to identify records missing key variables.
...
All of these missing _key_ values are in the `name` variable.
...
dee %>%
  ...
  labs(
    title = "Delaware Expenditures Amount Distribution",
...
There are no outlier dates, which makes sense given our records were returned
using a date range `POST()`.
...
dee %>% 
  ...
  labs(
    title = "Delaware Expenditures by Year",
    caption = "Source: {source}",
    fill = "Election Year",
    x = "Year Made",
    y = "Count"
...
raw_in <- percent(prop_in(dee$city_raw, valid_city))
progress %>% 
...
1. There are `r comma(nrow(dee))` records in the database.
1. There are `r comma(sum(dee$dupe_flag))` duplicate records in the database.
1. The range and distribution of `amount` and `date` seem reasonable.
1. There are `r comma(sum(dee$na_flag))` records missing key variables.
...
dict_raw <- tibble(
  var = md_code(names(dee)),
  old = c(md_code(old_names), rep("", ncol(dee) - length(old_names))),
  type = md_code(map_chr(dee, typeof)),
  def = c(
    ...
    "Expenditure amount",
    "Unique spending committee ID",
    "Spending committee name",
    "Expenditure category",
    "Purpose for expenditure",
    "Spending method",
    "Filing period reported",
