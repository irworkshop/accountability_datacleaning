...
doc_dir <- fs::dir_create(here::here("state", "or", "contribs", "docs"))
```
...
here::i_am("state/or/contribs/docs/or_contribs_diary.Rmd")
```
...
## Download
We downloaded the data from the [Oregon Secretary of State website][or_sos]. We filtered for everything between Jan 1, 2015 and Nov 12, 2022. 

[or_sos]:https://secure.sos.state.or.us/orestar/gotoPublicTransactionSearch.do?OWASP_CSRFTOKEN=4NSV-0VI2-IIZ1-4VJP-NZPP-2EEV-48A7-GS7B

```{r raw-dir}
raw_dir <- dir_create(here("state", "or", "contribs", "data", "raw"))
raw_path <- dir_ls(raw_dir,regexp = ".+2022.+")
```
## Read
...
```{r clean-names}
orc <- clean_names(orc, case = "snake")
```

## Explore

There are `r comma(nrow(orc))` rows of `r ncol(orc)` columns. Each record
...
orc <- flag_na(orc, all_of(key_vars))
sum(orc$na_flag)
```
...
orc$amount <- round(orc$amount, digits = 2)
```
...
```{r amount-minmax}
glimpse(orc[c(which.max(orc$amount), which.min(orc$amount)), ])
```
...
orc %>%
  ...
  labs(
    title = "Oregon Contributions Amount Distribution",
...
orc %>% 
  ...
  labs(
    title = "Oregon Contributions by Year",
    caption = "Source: {source}",
    fill = "Election Year",
    x = "Year Made",
    y = "Count"
...
```{r address-join}
orc <- left_join(orc, addr_norm, by = c("address1", "address2"))
```
...
raw_in <- percent(prop_in(orc$city, valid_city))
progress %>% 
...
```{r clean-timestamp}
min_dt <- str_remove_all(min(orc$date), "-")
max_dt <- str_remove_all(max(orc$date), "-")
csv_ts <- paste(min_dt, max_dt, sep = "-")
```

```{r clean-dir}
clean_dir <- dir_create(here("state", "or", "contribs", "data", "clean"))
clean_csv <- path(clean_dir, glue("or_contribs_{csv_ts}.csv"))
clean_rds <- path_ext_set(clean_csv, "rds")
basename(clean_csv)
```

```{r clean-write}
write_csv(orc, clean_csv, na = "")
write_rds(orc, clean_rds, compress = "xz")
