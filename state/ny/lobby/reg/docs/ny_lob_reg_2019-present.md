...
nylr <- nylr %>% 
 ...
 nylr <- nylr %>%  
  mutate(st_zip = str_extract(state_zip, "(?<=,\\s).+\\s\\d{5}"),
    cont_ben_client_state = str_extract(st_zip,".+(?=\\s\\d{5})"),     
    cont_client_zip = str_remove(st_zip, cont_ben_client_state) %>% str_trim(),
    addr_city = str_remove(state_zip, ",\\s.+\\s\\d{5}.*"),
    cont_ben_client_city = str_extract(addr_city, "(?<=\r\n)[^\r]+\r\n\r\n") %>% str_trim(),
    cont_client_addr = str_remove(addr_city, cont_ben_client_city) %>% str_trim()
      # separate(col, into = c("a", "b"), sep="\r\n(?=[^\r\n]+$)")
         ) %>% 
   select(-c(state_zip, addr_city, st_zip))
```
...
#> # … with 9 more variables: prin_lob_addr <chr>, cont_ben_client_state <chr>,
#> #   cont_client_zip <chr>, cont_ben_client_city <chr>, cont_client_addr <chr>,
#> #   ben_client_state <chr>, ben_client_zip <chr>, ben_client_city <chr>, ben_client_addr <chr>,
#> #   and abbreviated variable names ¹​prin_lob, ²​prin_lob_name, ³​co_sub_lob, ⁴​contractual_client,
#> #   ⁵​cont_client_name, ⁶​beneficial_client, ⁷​ben_client_name, ⁸​prin_lob_state, ⁹​prin_lob_zip,
#> #   ˟​prin_lob_city
tail(nylr)
#> # A tibble: 6 × 21
#>   year      prin_…¹ prin_…² ind_lob co_su…³ contr…⁴ cont_…⁵ benef…⁶ ben_c…⁷ prin_…⁸ prin_…⁹ prin_…˟
...
#> Rows: 20
#> Columns: 21
#> $ year                  <chr> "2021 - 2022", "2021 - 2022", "2021 - 2022", "2019 - 2020", "2019 -…
#> $ prin_lob              <chr> "RG GROUP \r\n\r\n29-12 161ST\r\nFLUSHING , NY 11358\r\n\r\n\r\n", …
#> $ prin_lob_name         <chr> "RG GROUP ", "NEW YORK STATE SOCIETY OF DERMATOLOGY AND DERMATOLOGI…
#> $ ind_lob               <chr> "CATHERINE GIULIANI \r\nALAN ROSENBERG \r\nGREGORY MITCHELL \r\n", …
#> $ co_sub_lob            <chr> NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA,…
#> $ contractual_client    <chr> "COMMVAULT SYSTEMS, INC.\r\n\r\n1 COMMVAULT WAY\r\nTINTON FALLS, NJ…
#> $ cont_client_name      <chr> "COMMVAULT SYSTEMS, INC.", "NEW YORK STATE SOCIETY OF DERMATOLOGY A…
#> $ beneficial_client     <chr> "COMMVAULT SYSTEMS, INC.\r\n\r\n1 COMMVAULT WAY\r\nTINTON FALLS, NJ…
#> $ ben_client_name       <chr> "COMMVAULT SYSTEMS, INC.", "NEW YORK STATE SOCIETY OF DERMATOLOGY A…
#> $ prin_lob_state        <chr> "NY", "NY", "NY", "NY", "NY", "NY", "NY", "NY", "NY", "NY", "NY", "…
#> $ prin_lob_zip          <chr> "11358", "12207", "10962", "12207", "10005", "12207", "11241", "114…
#> $ prin_lob_city         <chr> "FLUSHING", "ALBANY", "ORANGEBURGH", "ALBANY", "NEW YORK", "ALBANY"…
#> $ prin_lob_addr         <chr> "29-12 161ST", "136 STATE STREET", "2 BRIARWOOD LANE", "21 ELK STRE…
#> $ cont_ben_client_state <chr> "NJ", "NY", "NY", "CA", "NY", "NY", "NY", "NY", "NJ", "NY", "VA", "…
#> $ cont_client_zip       <chr> "07724", "12207", "10111", "92108", "10005", "11563", "10016", "146…
#> $ cont_ben_client_city  <chr> "TINTON FALLS", "ALBANY", "NEW YORK", "SAN DIEGO", "NEW YORK", "LYN…
#> $ cont_client_addr      <chr> "1 COMMVAULT WAY", "136 STATE STREET", "45 ROCKEFELLER PLAZA", "350…
#> $ ben_client_state      <chr> "NJ", "NY", "NY", "CA", "NY", "NY", "NY", "NY", "NJ", "NY", "VA", "…
#> $ ben_client_zip        <chr> "07724", "12207", "10111", "92108", "10005", "11563", "10016", "146…
#> $ ben_client_city       <chr> "TINTON FALLS", "ALBANY", "NEW YORK", "SAN DIEGO", "NEW YORK", "LYN…
#> $ ben_client_addr       <chr> "1 COMMVAULT WAY", "136 STATE STREET", "45 ROCKEFELLER PLAZA", "350…
```
...
#> 14 cont_ben_client_state <chr>    38 0.00262 
#> 15 cont_client_zip       <chr>    38 0.00262 
#> 16 cont_ben_client_city  <chr>     3 0.000207
...
#> 14 cont_ben_client_state <chr>    96 0.00663 
#> 15 cont_client_zip       <chr>  1817 0.125   
#> 16 cont_ben_client_city  <chr>  1191 0.0822  
...
#> Rows: 20
#> Columns: 32
#> $ year                       <chr> "2019 - 2020", "2021 - 2022", "2021 - 2022", "2019 - 2020", "2…
#> $ prin_lob                   <chr> "COZEN O'CONNOR PUBLIC STRATEGIES, LLC\r\n\r\n277 PARK AVENUE\…
#> $ prin_lob_name              <chr> "COZEN O'CONNOR PUBLIC STRATEGIES, LLC", "OSTROFF ASSOCIATES, …
#> $ ind_lob                    <chr> "KANA IO ", "REBECCA MARINO ", "RICHARD OSTROFF ", "JULIAN KLI…
#> $ co_sub_lob                 <chr> NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA…
#> $ contractual_client         <chr> "PRATT INSTITUTE\r\n\r\n200 WILLOUGHBY AVENUE\r\nBROOKLYN\r\nN…
#> $ cont_client_name           <chr> "PRATT INSTITUTE", "ARKER DIVERSIFIED COMPANIES", "NEW YORK IN…
#> $ beneficial_client          <chr> "PRATT INSTITUTE\r\n\r\n200 WILLOUGHBY AVENUE\r\nBROOKLYN\r\nN…
#> $ ben_client_name            <chr> "PRATT INSTITUTE", "ARKER DIVERSIFIED COMPANIES", "NEW YORK IN…
#> $ prin_lob_state             <chr> "NY", "NY", "NY", "NY", "NY", "NY", "NY", "NY", "NY", "NY", "N…
#> $ prin_lob_zip               <chr> "10172", "12207", "12207", "12207", "12207", "12207", "10271",…
#> $ prin_lob_city              <chr> "NEW YORK", "ALBANY", "ALBANY", "ALBANY", "ALBANY", "ALBANY", …
#> $ prin_lob_addr              <chr> "277 PARK AVENUE", "150 STATE STREET\r\nSUITE 301", "150 STATE…
#> $ cont_ben_client_state      <chr> "NY", "NY", "NY", "NY", "NY", "NY", "NY", "CT", "NY", "NY", "N…
#> $ cont_client_zip            <chr> "11205", "11576", "11231", "12504", "13203", "14454", "10022",…
#> $ cont_ben_client_city       <chr> "NEW YORK", "ROSLYN", "BROOKLYN", "ANNANDALE ON HUDSON", "SYRA…
#> $ cont_client_addr           <chr> "200 WILLOUGHBY AVENUE\r\nBROOKLYN", "1044 NORTHERN BLVD.", "C…
#> $ ben_client_state           <chr> "NY", "NY", "NY", "NY", "NY", "NY", "NY", "CT", "NY", "NY", "N…
#> $ ben_client_zip             <chr> "11205", "11576", "11231", "12504", "13203", "14454", "10022",…
#> $ ben_client_city            <chr> "NEW YORK", "ROSLYN", "BROOKLYN", "ANNANDALE ON HUDSON", "SYRA…
#> $ ben_client_addr            <chr> "200 WILLOUGHBY AVENUE\r\nBROOKLYN", "1044 NORTHERN BLVD.", "C…
#> $ dupe_flag                  <lgl> FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE,…
#> $ prin_lob_addr_norm         <chr> "277 PARK AVE", "150 STATE STREET SUITE 301", "150 STATE STREE…
#> $ cont_client_addr_norm      <chr> "200 WILLOUGHBY AVENUE BROOKLYN", "1044 NORTHERN BLVD", "CORP …
#> $ ben_client_addr_norm       <chr> "200 WILLOUGHBY AVENUE BROOKLYN", "1044 NORTHERN BLVD", "CORP …
#> $ cont_ben_client_state_norm <chr> "NY", "NY", "NY", "NY", "NY", "NY", "NY", "CT", "NY", "NY", "N…
#> $ ben_client_state_norm      <chr> "NY", "NY", "NY", "NY", "NY", "NY", "NY", "CT", "NY", "NY", "N…
#> $ prin_lob_city_norm         <chr> "NEW YORK", "ALBANY", "ALBANY", "ALBANY", "ALBANY", "ALBANY", …
#> $ cont_ben_client_city_norm  <chr> "NEW YORK", "ROSLYN", "BROOKLYN", "ANNANDALE ON HUDSON", "SYRA…
#> $ ben_client_city_norm       <chr> "NEW YORK", "ROSLYN", "BROOKLYN", "ANNANDALE ON HUDSON", "SYRA…
#> $ prin_lob_city_swap         <chr> "NEW YORK", "ALBANY", "ALBANY", "ALBANY", "ALBANY", "ALBANY", …
#> $ ben_client_city_swap       <chr> "NEW YORK", "ROSLYN", "BROOKLYN", "ANNANDALE ON HUDSON", "SYRA…
```
