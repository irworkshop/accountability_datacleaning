...
## Download
```{r raw_dir}
raw_dir <- dir_create(here("state","ia", "expends", "data", "raw"))
```
...
```{r check last update}
ia_prev <- read_csv(raw_dir %>% list.files(full.names = T))
```
...
ia <- ia %>% 
  mutate(payee_nm = coalesce(organization_nm, last_nm)) %>% 
  flag_na(committee_nm, payee_nm, date, amount)
...
ia <- flag_dupes(ia, -transaction_id)
sum(ia$dupe_flag)
mean(ia$dupe_flag)
```
...
```{r}
lookup_file <- here("ia", "expends", "data", "ia_city_lookup.csv")
if (file.exists(lookup_file)) {
  lookup <- read_csv(lookup_file) %>% select(1:2)
  ia <- left_join(ia, lookup)
  progress_table(
    str_to_upper(ia$city_raw),
    ia$city_norm,
    ia$city_swap,
