...
doc_dir <- fs::dir_create(here::here("state","ms", "expends", "docs"))
```
...
We can run an empty search using the start date and current dates. The next update should start on `2023-01-29`.
...
Since only one record was uploaded on Jan. 29, 2023 (time of this update), we only include records until Jan. 28, 2023. 
```{r}
...
We can see that most of the states not in our `valid_state` database are actually `NAs`, and it's safe to leave them as is. 
...
raw_in <- percent(prop_in(mse$city, valid_city))
progress %>% 
...
dict_raw <- tibble(
  ...
  def = c(
    ...
    "Receiving payee address",
    "Receiving payee city",
    "Receiving payee state",
    "Receiving payee zipcode",
    "Expenditure description",
    "Expenditure date",
    "Expenditure amount",
    "Flag indicating duplicate record",
    "Calendar year of date",
    "TAP-generated cleaned address",
    "TAP-generated cleaned city",
    "TAP-generated cleaned state",
    "TAP-generated cleaned zipcode"
  )
)
```
