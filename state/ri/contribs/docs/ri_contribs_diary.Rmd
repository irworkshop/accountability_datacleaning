...
```{r address-join}
ric <- left_join(ric, addr_norm, by = c("address1", "address2"))
```
...
```{r clean-timestamp}
min_dt <- str_remove_all(min(ric$date), "-")
max_dt <- str_remove_all(max(ric$date), "-")
