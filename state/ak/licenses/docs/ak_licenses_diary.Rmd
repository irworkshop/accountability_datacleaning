...
doc_dir <- fs::dir_create(here::here("state", "ak", "licenses", "docs"))
```
...
Professional licenses are available from the Alaska Department of Commerce, 
...
represents a single professional license issued to a business in Alaska.
...
explore_plot(akl, program) + scale_x_wrap()
explore_plot(akl, prof_type) + scale_x_wrap()
explore_plot(akl, status) + scale_x_wrap()
```
...
akl %>% 
  count(year_issued) %>% 
  mutate(even = is_even(year_issued)) %>% 
  ggplot(aes(x = year_issued, y = n)) +
...
raw_in <- percent(prop_in(akl$city, valid_city))
progress %>% 
