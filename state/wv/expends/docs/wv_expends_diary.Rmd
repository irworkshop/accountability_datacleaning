...
`r percent(wv$amount == 0, na.rm = TRUE)` of `amount` values are zero.
```{r zero_amount}
sum(wv$expenditure_amount < 0, na.rm = TRUE)
sum(wv$expenditure_amount == 0, na.rm = TRUE)
```
...
`r percent(prop_in(wv$zip, valid_zip, na.rm = TRUE))` of the values already in our 95% 
...
`r percent(prop_in(wv$state, valid_state, na.rm = TRUE))`.
...
`r percent(prop_in(wv$city_raw, valid_city, na.rm = TRUE))` already in `valid_city`, mostly due to case difference. If we simply convert to uppcase that numbers increases to 
`r percent(prop_in(str_to_upper(wv$city_raw), valid_city, na.rm = TRUE))`. We will aim to get this number over 99% using the other steps in the process.
...
`r percent(prop_na(wv$city_norm) - prop_na(wv$city_raw))`. These new `NA` values were either a single
...
`r percent(prop_in(wv$city_swap, valid_city, na.rm = TRUE))` and reduce the number of distinct
_invalid_ values from `r length(setdiff(wv$city_norm, valid_city))` to only
`r length(setdiff(wv$city_swap, valid_city))`
...
This brings us to `r percent(prop_in(wv$city_swap, valid_city, na.rm = TRUE))` valid values.
...
progress_table <- tibble(
  ...
  prop_good = c(
    ...
    prop_in(wv$city_norm, valid_city, na.rm = TRUE),
    prop_in(wv$city_swap, valid_city, na.rm = TRUE),
    prop_in(wv$city_clean, valid_city, na.rm = TRUE)
  ),
  total_distinct = c(
    n_distinct(str_to_upper(wv$city_raw)),
    n_distinct(wv$city_norm),
    n_distinct(wv$city_swap),
    n_distinct(wv$city_clean)
  ),
  unique_bad = c(
    length(setdiff(str_to_upper(wv$city_raw), valid_city)),
    length(setdiff(wv$city_norm, valid_city)),
    length(setdiff(wv$city_swap, valid_city)),
    length(setdiff(wv$city_clean, valid_city))
  )
)
...
1. There are `r nrow(wv)` records in the database.
1. There are `r sum(wv$dupe_flag)` duplicate records in the database.
1. The range and distribution of `amount` seems reasomable, and `date` has been cleaned by removing
`r sum(wv$date_flag, na.rm = T)` values from the distance past or future.
1. There are `r sum(wv$na_flag)` records missing either recipient or date.
