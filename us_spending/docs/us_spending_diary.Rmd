...
```{r include=FALSE}
head(spend_urls, 3)
head(str_replace(spend_zips, here(), "~"), 3)
```
...
First we will read `us_spend_checks.csv`, which contains information on the
number of rows, columns, unique types, missing, and zero values.

```{r echo=FALSE}
checks <- read_csv(
  file = "us_spend_checks.csv",
...
the data to a `us_spend_geo.csv` file.

```{r echo=FALSE}
geo <- read_csv(
  file = "us_spend_geo.csv",
