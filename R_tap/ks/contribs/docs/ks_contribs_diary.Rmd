...
doc_dir <- fs::dir_create(here::here("ks", "contribs", "docs"))
```
...
Kansas contribution data is downloaded from a [search portal][sp]. One has to
search for all contributions of a specific "type" (.e.g., check or cash). Those
...
There are `r comma(nrow(ksc))` rows of `r ncol(ksc)` columns. Each record
represents a single contribution from an individual or business to a candidate.
...
There are a significant number of duplicates but not enough to warrant removal.
...
explore_plot(ksc, typeof_tender)
explore_plot(ksc, industry)
explore_plot(mutate(ksc, across(occupation, toupper)), occupation)
```
...
ksc %>%
  ...
  labs(
    title = "Kansas Contributions Amount Distribution",
...
ksc %>% 
  ...
  labs(
    title = "Kansas Contributions by Year",
    caption = "Source: KS GEC",
    fill = "Election Year",
    x = "Year Made",
    y = "Count"
...
raw_in <- percent(prop_in(ksc$city, valid_city))
progress %>% 
