...
```{r}
for (look_for in c("Candidates", "PACs", "Lobbyist")) {
  message(look_for)
  csv_path <- path(raw_dir, look_for, ext = "csv")
    ...
    POST(
      ...
      body = c(
        ...
        list( # dropdown body arguments: ALL rows for each file type
          ...
          `ctl00$ContentPlaceHolder1$header1$ddlLookFor` = look_for,
...
for (type in c("Candidates", "PACs", "Lobbyists")) {
  message(type)
  type_csv <- path(raw_dir, paste0(type, ".csv"))
  POST(
    url = "https://www.cfis.state.nm.us/media/CFIS_Data_Download.aspx",
    write_disk(path = type_csv, overwrite = TRUE),
    set_cookies(nm_cookie),
    progress("down"),
    body = list(
