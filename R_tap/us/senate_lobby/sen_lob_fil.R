...
# arguments ---------------------------------------------------------------
# Argument order:
#   1. CSV file path to continuously save rows
#   2. Date after which filings are posted (optional)
#   3. The alphanumeric API key (optional)

# Rscript sen_lob_fil.R --args {output file} {from date} {api key}
# Rscript sen_lob_fil.R --args sen_lob.csv 2021-01-01 abcdefg123456
...
if (length(cmd_args) >= 1 && grepl("csv$", cmd_args[1])) {
  ...
  lob_csv <- file.path(getwd(), "senate_lobbyist_filings.csv")
}

cli_alert_info("Saving filings to {.path {lob_csv}}")
...
if (length(cmd_args) >= 3) {
  ...
  cli_alert_info("Using API key: {col_grey(api_key)}")
} else {
...
fil_get <- GET(
  ...
  query = list(page_size = 250, filing_dt_posted_after = dt_from),
  auth_lda()
)
...
while (has_next) {
  ...
  write_csv(fil_dat$results, file = lob_csv, append = TRUE, progress = FALSE)
  n_row <- n_row + nrow(fil_dat$results)
  prop_row <- paste0(round(100 * (n_row/all_row), 2), "%")
  cli_alert_success("Total results written: {n_row} ({prop_row})")
}
