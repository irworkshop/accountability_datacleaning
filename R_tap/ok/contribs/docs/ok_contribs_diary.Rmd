...
Oklahoma contributions can be downloaded from the  [Oklahoma Ethics Commission's
website][oec] as a ZIP archive.

[oec]: https://www.ok.gov/ethics/public/login.php
...
```{r}
desc_doc <- file_temp(ext = "doc")
desc_get <- GET(
  url = "https://www.ok.gov/ethics/public/dfile.php",
  query = list(action = "descriptions"),
  write_disk(desc_doc),
  progress("down")
)
...
raw_zip <- path(raw_dir, "ethicscsvfile.zip")
```
...
transaction <- read_csv(
  file = path(raw_dir, "transaction.csv"),
  name_repair = make_clean_names,
  col_select = list(-last_col()),
  col_types = cols(
...
```{r read_contributor}
contributor <- read_csv(
  file = path(raw_dir, "contributor.csv"),
  name_repair = make_clean_names,
  col_select = list(-last_col()),
  col_types = cols(
...
```{r read_cont_type}
cont_type <- read_csv(
  file = path(raw_dir, "cont_type.csv"),
  name_repair = make_clean_names,
  col_select = list(-last_col()),
  col_types = cols(
...
individual_cont <- read_csv(
  file = path(raw_dir, "individual_cont.csv"),
  name_repair = make_clean_names,
  col_select = list(-last_col()),
  col_types = cols(
    contributor_id = col_integer(),
...
business_cont <- read_csv(
  file = path(raw_dir, "business_cont.csv"),
  name_repair = make_clean_names,
  col_select = list(-last_col()),
  col_types = cols(
    cont_id = col_double(),
...
committee_cont <- read_csv(
  file = path(raw_dir, "committee_cont.csv"),
  name_repair = make_clean_names,
  col_select = list(-last_col()),
  col_types = cols(
...
```{r read_vendor_cont}
vendor_cont <- read_csv(
  file = path(raw_dir, "vendor_cont.csv"),
  name_repair = make_clean_names,
  col_select = list(-last_col()),
  col_types = cols(
    cont_id = col_integer(),
...
```{r read_district}
district <- read_csv(
  file = path(raw_dir, "district.csv"),
  name_repair = make_clean_names,
  col_select = list(-last_col()),
  col_types = cols(
...
```{r read_candidate}
candidate <- read_csv(
  file = path(raw_dir, "candidate.csv"),
  name_repair = make_clean_names,
  col_select = list(-last_col()),
  col_types = cols(
    .default = col_character(),
...
```{r read_lump_fund}
lump_fund <- read_csv(
  file = path(raw_dir, "lump_fund.csv"),
  name_repair = make_clean_names,
  col_select = list(-last_col()),
  col_types = cols(
    rep_num = col_integer(),
...
```{r read_rep_type}
report_type <- read_csv(
  file = path(raw_dir, "report_type.csv"),
  name_repair = make_clean_names,
  col_select = list(-last_col()),
  col_types = cols(
    report_type = col_integer(),
...
First, we will join the `contributors` table, which contains geographic data on
...
1. Individuals with `cont_fname` (first), `cont_mname` (middle), and
`cont_lname` (last)
    * With `cont_employer` and `cont_occupation`
2. Businesses with a `cont_bname`
    * With `cont_activity`
3. Committees with a `cont_cname`
    * with `cont_interest` and `ethics_id`
4. Vendors with a `cont_vname`
...
There appears to be more total contributors than listed in the contributors
database. There are `r nrow(individual_conts) - nrow(contributors)` more
_individual_ contributors alone than there are total records in the contributors
database.
...
There are `r nrow(all_recipients)` unique committees that have filed SO-1 or
