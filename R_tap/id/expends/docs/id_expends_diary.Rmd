...
doc_dir <- fs::dir_create(here::here("id", "expends", "docs"))
```
...
here::i_am("id/expends/docs/id_expends_diary.Rmd")
```
...
```{r clean-names}
ide <- clean_names(ide, case = "snake")
```

## Explore

There are `r comma(nrow(ide))` rows of `r ncol(ide)` columns. Each record
...
ide <- flag_na(ide, all_of(key_vars))
sum(ide$na_flag)
```
...
ide$amount <- round(ide$amount, digits = 2)
```
...
```{r amount-minmax}
glimpse(ide[c(which.max(ide$amount), which.min(ide$amount)), ])
```
...
```{r address-join}
ide <- left_join(ide, addr_norm, by = c("address1", "address2"))
```
...
```{r clean-timestamp}
min_dt <- str_remove_all(min(ide$date), "-")
max_dt <- str_remove_all(max(ide$date), "-")
...
dict_raw <- tibble(
  var = md_code(names(ide)),
  type = md_code(map_chr(ide, typeof)),
  def = c(
    ""
  )
)
